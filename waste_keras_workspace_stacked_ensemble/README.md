...
## Version 2: stacked meta-ensemble of all 8 transfer-learning models

This version adds a **stacked meta-ensemble** after training the eight CNN backbones. Each base model produces a probability vector for every image. The eight vectors are concatenated and used as features for a multinomial **logistic-regression meta-classifier**.

Base learners: MobileNetV2, EfficientNetB0, ResNet50, DenseNet121, InceptionV3, Xception, NASNetMobile, and VGG16.

The stacker is fit on **validation predictions only**. The test split remains untouched until final ensemble evaluation. This is important: fitting the stacker on test predictions would leak test information. For research-grade estimates, an out-of-fold stacking scheme is preferable; this workspace uses validation stacking because it is easier to understand and run in a two-hour/bachelor-level applied workflow.

New outputs are written to `outputs/StackedMetaEnsemble/`:

- `meta_classifier.joblib` — trained meta learner
- `ensemble_manifest.json` — model order/classes needed for inference
- `metrics.json` — validation and test metrics
- `classification_report.csv`
- `confusion_matrix.png`
- `base_model_contributions.csv`
- `base_model_contributions.png`

Run the full experiment exactly as before:

```bash
python train_all.py --config configs/config.yaml
```

Predict one image with the ensemble:

```bash
python predict_ensemble.py path/to/waste_image.jpg --outputs outputs
```

Set `ensemble.enabled: false` in `configs/config.yaml` to disable stacking.
